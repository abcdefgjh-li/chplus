    ...
    if (match(TokenType::DEFINE)) {
        ...
        if (match(TokenType::LPAREN)) {
            if (match(TokenType::STRUCT)) {
                ...
                // 不是"定义(结构体)"，可能是"定义(结构体类型名)"，继续解析变量定义
                // 不回退，直接调用parseVariableDef，它会从当前位置继续解析
            ...
            while (match(TokenType::COMMA)) {
                ...
                if (nextParamToken.type == TokenType::DEFINE) {
                    ...
                    if (nextTypeToken.type == TokenType::INTEGER) {
                        ...
                        nextParamType = "布尔型";
                    } else if (nextTypeToken.type == TokenType::VOID) {
                        nextParamType = "空类型";
                    } else if (nextTypeToken.type == TokenType::IDENTIFIER) {
                        // 检查是否是结构体类型
                        nextParamType = nextTypeToken.value;
                        advance(); // 消费类型token
                    } else {
                        throw std::runtime_error("无效的参数类型 在第 " + std::to_string(nextTypeToken.line) + " 行");
                    }
                    if (nextTypeToken.type != TokenType::IDENTIFIER) {
                        advance(); // 消费类型token
                    }
...
// 解析乘方表达式
std::unique_ptr<ASTNode> Parser::parsePowerExpression() {
    ...
    if (match(TokenType::IDENTIFIER)) {
        ...
        if (match(TokenType::LBRACKET)) {
            ...
            if (match(TokenType::ASSIGN)) {
                auto expr = parseExpression();
                // 创建多维数组访问赋值节点
                return std::make_unique<ArrayAssignmentNode>(name, std::move(indices), std::move(expr), previous().line, previous().column);
            } else {
                // 如果只有一个索引且没有额外维度，可能是字符串索引访问
                if (indices.size() == 1 && peek().type != TokenType::LBRACKET) {
                    // 创建字符串索引访问节点
                    return std::make_unique<StringAccessNode>(name, std::move(indices.at(0)), previous().line, previous().column);
                } else {
                    // 创建多维数组访问节点
                    return std::make_unique<ArrayAccessNode>(name, std::move(indices), previous().line, previous().column);
                }
            }
        }
        ...
        if (match(TokenType::DOT)) {
            ...
            if (match(TokenType::ASSIGN)) {
                auto expr = parseExpression();
                // 创建一个特殊的赋值节点来处理结构体成员赋值
                return std::make_unique<StructMemberAssignmentNode>(std::move(structExpr), memberName, std::move(expr), previous().line, previous().column);
            } else {
...
// 解析else if语句
std::unique_ptr<ASTNode> Parser::parseElseIfStatement() {
    int line = previous().line;
    int column = previous().column;
    
    consume(TokenType::LPAREN, "else if语句必须以 '(' 开始");
    auto condition = parseExpression();
    consume(TokenType::RPAREN, "else if条件必须以 ')' 结束");
    
    // 解析else if语句体
    auto thenBranch = parseStatement();
    
    // 解析后续的else if或else分支（可选）
    std::unique_ptr<ASTNode> elseBranch = nullptr;
    if (match(TokenType::ELSE_IF)) {
        // 递归解析else if语句
        elseBranch = parseElseIfStatement();
    ...
    // 创建else if语句节点
    return std::make_unique<ElseIfStatementNode>(std::move(condition), std::move(thenBranch), 
                                               std::move(elseBranch), line, column);
}
...
std::unique_ptr<ASTNode> Parser::parseReturnStatement() {
    ...
    if (match(TokenType::SEMICOLON)) {
        // 创建空的返回语句节点
        return std::make_unique<ReturnStatementNode>(nullptr, line, column);
    }
...
std::unique_ptr<ASTNode> Parser::parseImportStatement() {
    ...
    auto filePathExpr = parseExpression();
    consume(TokenType::RPAREN, "导入语句必须以 ')' 结束");
    consume(TokenType::SEMICOLON, "语句必须以分号结束");
    
    // 获取文件路径字符串
    if (filePathExpr->type != NodeType::LITERAL) {
        throw std::runtime_error("导入语句的文件路径必须是字符串字面量 在第 " + std::to_string(line) + " 行");
    }
    
    LiteralNode* literal = static_cast<LiteralNode*>(filePathExpr.get());
...
std::unique_ptr<ASTNode> Parser::parseStructDef() {
    ...
    // 结构体关键字已经由 match(TokenType::STRUCT) 消耗
    // 现在消费结构体名称
