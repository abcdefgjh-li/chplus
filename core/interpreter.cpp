...
std::string inferType(const std::string& value) {
    ...
    // 检查布尔类型
    if (value == "真" || value == "假" || value == "true" || value == "false") {
        return "布尔型";
    }
...
// 符号表构造函数
SymbolTable::SymbolTable(SymbolTable* parent, FileMemory* fileMem, bool useFileMem, MemoryStorage* memStorage, bool useMemStorage)
    : parent(parent), fileMemory(fileMem), memoryStorage(memStorage), useFileMemory(useFileMem), useMemoryStorage(useMemStorage) {}
...
void SymbolTable::defineVariable(const std::string& name, const std::string& type, const std::string& value, int line, bool initArrayElements) {
    ...
    // 如果使用文件内存存储，同时保存到文件
    if (useFileMemory && fileMemory != nullptr) {
        fileMemory->defineVariable(name, type, value);
    }
    
    // 如果使用纯内存存储，同时保存到内存
    if (useMemoryStorage && memoryStorage != nullptr) {
        memoryStorage->defineVariable(name, type, value);
    }
    ...
    if (name.find("[") != std::string::npos && name.find("]") != std::string::npos && initArrayElements) {
        ...
        while (pos < dimsStr.length()) {
            if (dimsStr[pos] == '[') {
                ...
                try {
                    long long dimensionSize = std::stoll(sizeStr);
                    if (dimensionSize <= 0) {
                        throw std::runtime_error("数组维度大小必须大于0: " + sizeStr);
                    }
                    if (!useFileMemory && !useMemoryStorage && dimensionSize > 1000) {
                        throw std::runtime_error("数组维度大小过大，最大允许1000: " + sizeStr);
                    }
                    dimensions.push_back(static_cast<int>(dimensionSize));
...
void SymbolTable::setVariable(const std::string& name, const std::string& value, int line) {
    if (variables.find(name) != variables.end()) {
        ...
        // 如果使用文件内存存储，同时更新文件
        if (useFileMemory && fileMemory != nullptr) {
            fileMemory->setVariable(name, value);
        }
        // 如果使用纯内存存储，同时更新内存
        if (useMemoryStorage && memoryStorage != nullptr) {
            memoryStorage->setVariable(name, value);
        }
    } else if (parent) {
...
std::string SymbolTable::getVariable(const std::string& name, int line) const {
    ...
    } else if (useFileMemory && fileMemory != nullptr && fileMemory->hasVariable(name)) {
        return fileMemory->getVariable(name);
    } else if (useMemoryStorage && memoryStorage != nullptr && memoryStorage->hasVariable(name)) {
        return memoryStorage->getVariable(name);
    } else if (parent) {
...
std::string SymbolTable::getVariableType(const std::string& name) const {
    // 首先尝试精确匹配
    if (variables.find(name) != variables.end()) {
        std::string varType = variables.at(name).first;
        // 检查变量名是否包含数组维度
    ...
    // 如果找不到精确匹配，尝试查找包含该名称前缀的数组变量
    // 例如：查找"阶乘数组"时，尝试查找"阶乘数组[100]"
    for (const auto& pair : variables) {
        const std::string& varName = pair.first;
        if (varName.find(name) == 0 && varName.length() > name.length() && varName[name.length()] == '[') {
            // 找到匹配的数组变量
            std::string varType = pair.second.first;
            varType = varType + "_数组";
            return varType;
        }
    }
...
void SymbolTable::defineFunction(FunctionDefNode* function) {
    ...
    // 提取函数签名（包含参数类型和数组维度信息）
    std::vector<std::string> paramTypes;
    for (const auto& param : function->parameters) {
        std::string typeWithArray = param.first;
        // 检查参数名是否包含数组维度
        if (param.second.find('[') != std::string::npos) {
            typeWithArray = param.first + "_数组";
        }
        paramTypes.push_back(typeWithArray);
    }
    ...
    for (const auto& existingFunc : functions[funcName]) {
        ...
        for (const auto& param : existingFunc->parameters) {
            ...
            existingParamTypes.push_back(typeWithArray);
        }
...
SymbolTable* SymbolTable::enterScope() {
    return new SymbolTable(this, fileMemory, useFileMemory, memoryStorage, useMemoryStorage);
}
...
// 执行器构造函数
Interpreter::Interpreter(std::unique_ptr<ProgramNode> program, bool debug, bool useFileMem, bool useMemStorage, bool reserveMem)
    : program(std::move(program)), globalScope(new SymbolTable(nullptr, nullptr, false, nullptr, false)), 
      debugMode(debug), useFileMemory(useFileMem), useMemoryStorage(useMemStorage), fileMemory(nullptr), memoryStorage(nullptr), reserveMemory(reserveMem) {
    // 初始化全局作用域
    if (useFileMemory) {
        // 清除旧的内存文件
        std::remove("memory.txt");
        fileMemory = new FileMemory("memory.txt", reserveMemory);
        globalScope = new SymbolTable(nullptr, fileMemory, true, nullptr, false);
    } else if (useMemoryStorage) {
        // 使用纯内存存储
        memoryStorage = new MemoryStorage();
        globalScope = new SymbolTable(nullptr, nullptr, false, memoryStorage, true);
    }
}
                    ...
                    if (varDef->isArray) {
                        ...
                        if (varDef->arraySizeExpr) {
                            ...
                            try {
                                ...
                                if (!useFileMemory && !useMemoryStorage && size > 1000) {
...
void Interpreter::executeStatement(ASTNode* node, SymbolTable* scope, const std::string& expectedReturnType) {
    ...
    switch (node->type) {
        ...
        case NodeType::COMPOUND_ASSIGNMENT: {
            ...
            if (isNumeric) {
                BigInt left(currentValue);
                BigInt right(exprValue);
                BigInt result;
                ...
                if (compoundAssign->op == "+") {
                    ...
                    if (right.isZero()) {
                        throw std::runtime_error("取模除零错误 在第 " + std::to_string(node->line) + " 行");
                    }
                    result = left % right;
                } else if (compoundAssign->op == "^") {
                    result = left ^ right;
    ...
    switch (node->type) {
        ...
        case NodeType::FUNCTION_CALL: {
            ...
            for (const auto& argNode : funcCall->arguments) {
                ...
                // 调试信息
                std::cout << "[调试] 参数类型推断: argNode->type=" << static_cast<int>(argNode->type) << std::endl;
                
                // 改进的类型推断：优先使用变量的声明类型
                if (argNode->type == NodeType::IDENTIFIER) {
                    // 如果是变量引用，使用变量的声明类型
                    IdentifierNode* ident = static_cast<IdentifierNode*>(argNode.get());
                    std::cout << "[调试] 变量名: " << ident->name << ", hasVariable=" << scope->hasVariable(ident->name) << std::endl;
                    if (scope->hasVariable(ident->name)) {
                        std::string varType = scope->getVariableType(ident->name);
                        std::cout << "[调试] 变量类型: " << varType << std::endl;
                        argumentTypes.push_back(varType);
                    } else {
                        // 如果变量不存在，使用值的类型推断
                        std::string inferred = inferType(argValue);
                        std::cout << "[调试] 推断类型: " << inferred << std::endl;
                        argumentTypes.push_back(inferred);
                    }
                } else if (argNode->type == NodeType::ARRAY_ACCESS) {
                    // 如果是数组访问，使用数组的声明类型
                    ArrayAccessNode* arrayAccess = static_cast<ArrayAccessNode*>(argNode.get());
                    std::cout << "[调试] 数组名: " << arrayAccess->arrayName << ", hasVariable=" << scope->hasVariable(arrayAccess->arrayName) << std::endl;
                    std::string varType = scope->getVariableType(arrayAccess->arrayName);
                    std::cout << "[调试] 数组类型: " << varType << std::endl;
                    argumentTypes.push_back(varType);
                } else {
        ...
        case NodeType::UNARY_EXPRESSION: {
            ...
            if (unaryExpr->op == "-") {
                try {
                    BigInt numValue(operand);
                    BigInt result = BigInt(0) - numValue;
            ...
            if (unaryExpr->op == "前缀++") {
                ...
                // 获取当前值
                BigInt currentValue(operand);
                BigInt newValue = currentValue + BigInt(1);
                
                // 设置新值
                std::string newValueStr = newValue.toString();
        ...
        case NodeType::BINARY_EXPRESSION: {
            ...
            if (binExpr->op == "+") {
                if (leftIsNumber && rightIsNumber && !left.empty() && !right.empty()) {
                    BigInt result = leftValue + rightValue;
                    return result.toString();
                } else {
                    // 有非数字，进行字符串拼接
                    return left + right;
            ...
            if (binExpr->op == "&&" || binExpr->op == "和" || binExpr->op == "且") {
                bool leftTrue = (left == "true" || left == "真" || (leftIsNumber && !leftValue.isZero()));
                bool rightTrue = (right == "true" || right == "真" || (rightIsNumber && !rightValue.isZero()));
